...
import { isProgramStudioAuthenticated, requireProgramStudio } from "@/lib/program/auth";
...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({ programs: listWeeklyPrograms() });
}
...
export async function HEAD() {
  ...
  return new NextResponse(null, { status: authed ? 200 : 401 });
}
